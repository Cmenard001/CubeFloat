 ...
 * @author	Luc Hérault
 * @date	May 2024
 * @brief	Module pour utiliser le BMP180
 *******************************************************************************
 * @verbatim
 * |----------------------------------------------------------------------
 ...
 * Un portage est ici réalisé pour STM32G431
 *
 * @endverbatim
 */
#include "config.h"
...
/*
 * Si vous avez relié la broche Vcc du BMP180 à l'un des GPIO du microcontrôleur,
 * cette fonction d'init peut s'en servir pour assurer un état connnu du capteur en forçant le reset de son alimentation
 * Dans le cas contraire, les paramètres GPIOx et GPIO_Pin peuvent être mis à NULL et 0 respectivement
 */
BMP180_Result_t BSP_BMP180_Init(BMP180_t* BMP180_Data, GPIO_TypeDef * GPIOx, uint16_t GPIO_Pin) {
	...
	if (GPIOx != NULL && GPIO_Pin != 0) {
		BSP_GPIO_pin_config(GPIOx, GPIO_Pin, GPIO_MODE_OUTPUT_PP, GPIO_NOPULL, GPIO_SPEED_FAST, GPIO_NO_AF);
		HAL_GPIO_WritePin(GPIOx, GPIO_Pin, GPIO_PIN_RESET);
		HAL_Delay(10);	//Force reset for 10ms... to ensure the BMP180 state
	...
	/* Initialize I2C */
	BSP_I2C_Init(BMP180_I2C, BMP180_I2C_SPEED, true);
	/* Test if device is connected */
	if (!BSP_I2C_IsDeviceConnected(BMP180_I2C, BMP180_I2C_ADDRESS)) {
	...
	BSP_I2C_ReadMulti(BMP180_I2C, BMP180_I2C_ADDRESS, BMP180_REGISTER_EEPROM, data, 22);
...
BMP180_Result_t BSP_BMP180_StartTemperature(BMP180_t* BMP180_Data) {
	...
	/* Send to device */
	BSP_I2C_Write(BMP180_I2C, BMP180_I2C_ADDRESS, BMP180_REGISTER_CONTROL, BMP180_COMMAND_TEMPERATURE);
	/* Set minimum delay */
...
BMP180_Result_t BSP_BMP180_ReadTemperature(BMP180_t* BMP180_Data) {
	...
	BSP_I2C_ReadMulti(BMP180_I2C, BMP180_I2C_ADDRESS, BMP180_REGISTER_RESULT, data, 2);
...
BMP180_Result_t BSP_BMP180_StartPressure(BMP180_t* BMP180_Data, BMP180_Oversampling_t Oversampling) {
	...
	/* Send to device */
	BSP_I2C_Write(BMP180_I2C, BMP180_I2C_ADDRESS, BMP180_REGISTER_CONTROL, command);
...
BMP180_Result_t BMP180_ReadPressure(BMP180_t* BMP180_Data) {
	...
	/* Read multi bytes from I2C */
	BSP_I2C_ReadMulti(BMP180_I2C, BMP180_I2C_ADDRESS, BMP180_REGISTER_RESULT, data, 3);
